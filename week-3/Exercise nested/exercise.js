...
// What is the name of the 4th Person?
console.log(people[3].name.first + " " +people[3].name.middle +" "+ people[3].name.last)
// How many children does the 3rd Person have?
console.log(people[2].family.children.length)
// What Street does the 7th Person live on?
console.log(people[6].address.street)
// How many people are over the age of 30 (Exclude the children & spouses)?
...
console.log(count)
// What is the avrg age of all the spouses?
const sumSp = people.reduce(
    (accumulator,person) => {
        if(person.family.spouse)
        return person.family.spouse.age + accumulator
        else
            return accumulator
    },
    0,
);
const countSp = people.reduce(
    (accumulator,person) => {
        if(person.family.spouse)
...
console.log(sumSp/countSp)

let sumS = 0
let countS = 0
for (const p of people) {
    if( p.family.spouse){
    sumS += p.family.spouse.age
    countS++ 
    }
}
console.log(sumS/countS)
// How many people are there? (People, Spouses, Children)
let counter = 0
for (const p of people) {
    counter ++
    if( p.family.spouse){
    ...
    if( p.family.children){
        counter+=p.family.children.length
    }
}
